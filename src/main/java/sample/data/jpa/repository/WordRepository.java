...
import sample.data.jpa.domain.WordBean;
...
public interface WordRepository extends CrudRepository<Word, Long> {
	...
	@Query("select new sample.data.jpa.domain.WordSummary(w, 0, sum(uaa.hit)) from Word w left join w.articles uaa where uaa.article.user.id = :id group by w.id order by uaa.hit desc ")
	// and uwa.user.id = :id
	...
	@Query(value = "select t1.word_id id, t1.value content, t1.explain2, t1.pron, t1.hit, uwo.rank, uwo.user_id from ("
			...
			+ "	group by w.id "
			+ " ) as t1 left join user_word_asso uwo on t1.word_id = uwo.word_id", name = "testResult", nativeQuery = true)
	List<WordBean> findByUserId2(@Param("id") Long userId); // , Pageable pageable

	@Query(value = "select t1.word_id id, t1.word_id id_sort, t1.value, t1.value value_sort, t1.create_date, t1.explain2, t1.explain2 explain_sort, t1.level, t1.pron, t1.pron pron_sort, t1.last_upt, t1.low_value, t1.mark, t1.hit temp_hit, t1.hit hit_sort, "
			...
			+ "limit :page, :size", name = "testResult", nativeQuery = true)
	 ...
	 * @return
	 */
	// @Query("select count(*) from Word w left join w.articles uaa where uaa.article.user.id = :id   ")
	// group by w.id

	@Query(value = "select count(*)  from ("
			+ " select w.id word_id, w.value, w.explain2, w.pron, sum(awa.hit) hit, w.low_value, w.create_date, w.last_upt, w.mark, a.user_id from word w,  article a , article_word_asso awa "
			+ " where w.id = awa.word_id and a.id = awa.article_id and a.user_id = :id "
			+ "	group by w.id "
			+ " ) as t1 left join user_word_asso uwo on t1.word_id = uwo.word_id", name = "testResult", nativeQuery = true)
